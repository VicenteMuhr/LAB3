...
class Dataset { //Clase Dataset según lo estipulado por el laboratorio.
    ...
    ArrayList<Game> getGamesByPrice(int price) {
        ...
        // CAMBIO MÍNIMO: Usar equalsIgnoreCase para flexibilidad
        if ("Price".equalsIgnoreCase(sortedByAttribute)) {
    ...
    int BinarySearchByPrice(int price) {
        ...
        while (low <= high) {
            // CAMBIO MÍNIMO: Cálculo de 'mid' para evitar posible desbordamiento en enteros grandes
    ...
    ArrayList<Game> getGamesByCategory(String Category){
        ...
        // CAMBIO MÍNIMO: Usar equalsIgnoreCase para flexibilidad
        if("category".equalsIgnoreCase(sortedByAttribute)){
    ...
    ArrayList<Game> getGamesByQuality(int Quality){
        ArrayList<Game> results = new ArrayList<>();
        // CAMBIO MÍNIMO: Usar equalsIgnoreCase para flexibilidad
        if("quality".equalsIgnoreCase(sortedByAttribute)){
    ...
    private void countingSort(String attribute, int maxValue){
        // CAMBIO MÍNIMO: Asegurar que Counting Sort solo se aplique a 'quality'
        if (!"quality".equalsIgnoreCase(attribute)) {
            System.out.println("ADVERTENCIA: Counting Sort solo está implementado para el atributo 'quality'. No se aplicará ordenamiento.");
            return;
        }
        ...
        Arrays.fill(count, 0); // CAMBIO MÍNIMO: Importar Arrays para usar Arrays.fill
        for(int i = 0; i < n; i++){
            int value;
            // CAMBIO MÍNIMO: Solo obtener valor de calidad para Counting Sort
    ...
    void sortByAlgorithm(String algorithm, String attribute){
        ...
        // CAMBIO MÍNIMO: Usar toLowerCase para que la selección del atributo sea flexible
        ...
        // CAMBIO MÍNIMO: Usar toLowerCase para que la selección del algoritmo sea flexible
        switch(algorithm.toLowerCase()){
            ...
            case "quicksort":
                ...
                break;
            // CAMBIO MÍNIMO: Integrar Counting Sort
            case "countingsort":
                // Counting Sort solo es aplicable a "quality" en este contexto
                if ("quality".equalsIgnoreCase(attribute)) {
                    countingSort("quality", 100); // Rango de calidad es 0-100
                } else {
                    System.out.println("ADVERTENCIA: Counting Sort solo se puede aplicar al atributo 'quality'. Usando Collections.sort como fallback.");
                    Collections.sort(Data, comparator);
                }
                break;
            case "collections.sort": // Opción para usar el método nativo de Java
                Collections.sort(Data, comparator);
                break;
            default:
                Collections.sort(Data, comparator);
        ...
        // CAMBIO MÍNIMO: Almacenar el atributo en minúsculas para consistencia
...
class GenerateData { //Clase GenerateData según lo estipulado por el laboratorio.
    // CAMBIO MÍNIMO: CATEGORIES según la especificación del Lab 3 (página 4)
    public static final String[] CATEGORIES = {"Acción", "Aventura", "Estrategia", "RPG",
            "Deportes", "Simulación"};
    ...
    public static ArrayList<Game> generateRandomGames(int numberOfGames) {
        ...
        for(int i = 0; i < numberOfGames; i++){
            ...
            String generatedName = PREFIXES[random.nextInt(PREFIXES.length)] + " "
                    ...
                    + i; // Añadido 'i' para nombres más únicos
            // CAMBIO MÍNIMO: Generación de precio según el rango del laboratorio (1,000 a 70,000)
            int generatedPrice = random.nextInt(70000 - 1000 + 1) + 1000;
    ...
    public static void saveGamesToCsv(ArrayList<Game> games, String filename){
        try(BufferedWriter bw = new BufferedWriter(new FileWriter(filename))){
            ...
            bw.newLine();
            for(Game game : games){
                // CAMBIO MÍNIMO: Manejo de comillas en el nombre para CSV
                bw.write(game.getCategory() + "," +
                        game.getQuality() + ",\"" +
                        game.getName().replace("\"", "\"\"") + "\"," + // Escapar comillas dobles en el nombre
                        game.getPrice());
                bw.newLine();
            }
    ...
    // CAMBIO MÍNIMO: Método para cargar desde CSV, necesario para el demo y los benchmarks
    public static ArrayList<Game> loadGamesFromCsv(String filename) {
        ...
        try (BufferedReader br = new BufferedReader(new FileReader(filename))) {
            ...
            while ((line = br.readLine()) != null) {
                // Regex para dividir por comas, pero no si están dentro de comillas
                String[] parts = line.split(",(?=(?:[^\"]*\"[^\"]*\")*[^\"]*$)");
                if (parts.length == 4) {
                    try {
                        String category = parts[0];
                        int quality = Integer.parseInt(parts[1]);
                        String name = parts[2].replace("\"", ""); // Remover comillas dobles
                        int price = Integer.parseInt(parts[3]);
                        games.add(new Game(category, quality, name, price));
                    } catch (NumberFormatException e) {
                        System.err.println("Saltando línea mal formada (problema de número): " + line);
                    }
                } else {
                    System.err.println("Saltando línea mal formada (cantidad de partes incorrecta): " + line);
                }
            }
            System.out.println("Dataset cargado de: " + filename + " (" + games.size() + " juegos)");
        } catch (IOException e) {
            System.err.println("Error al cargar el dataset de CSV '" + filename + "': " + e.getMessage());
        }
        return games;
    }
}
public class Main {
    ...
    public static void main(String[] args) {
        ...
        // CAMBIO MÍNIMO: Llamar a loadGamesFromCsv de GenerateData
        ArrayList<Game> loadedGames = GenerateData.loadGamesFromCsv(demoFilename);
        ArrayList<Game> orderedDemoGames = new ArrayList<>(loadedGames); // Usar loadedGames
        Dataset demoDataset = new Dataset(orderedDemoGames);

        String[] algorithms = {"bubbleSort", "insertionSort", "selectionSort", "mergeSort",
                "quickSort", "countingSort", "Collections.sort"}; // CAMBIO MÍNIMO: Añadido "Collections.sort" y "countingSort"
        ...
        if (demoDataset.Data.isEmpty()) {
            ...
            // CAMBIO MÍNIMO: Usar toLowerCase para coincidir con el sortedByAttribute
            switch (chosenAttribute.toLowerCase()) {
                case "price":
        ...
        //Poner entre /* */ junto con el resto del main actual,
        //Si se quiere usar el experimental.
        //También, se usa mergeSort con búsqueda binaria,
        //ya que mergeSort soporta los 10^6 datos.
        System.out.println("-----------------------------------------");
        ...
        System.out.printf("getGamesByPrice (Lineal): %.3f ms, Encontrados: %d juegos\n",
...
// Binary
        ...
        System.out.printf("getGamesByPrice (Binaria): %.3f ms, Encontrados: %d juegos\n",
                durationPriceBinaryMs, resultsPriceBinary.size());

        System.out.println("\n--- getGamesByPriceRange ---");
        int lowerPrice = 20;
        int higherPrice = 60;
        System.out.println("Buscando por rango de precios: [" + lowerPrice + ", " + higherPrice + "]");
        ...
        System.out.printf("getGamesByPriceRange (Lineal): %.3f ms, Encontrados: %d juegos\n",
        ...
        System.out.println("Ordenando por 'price' para búsqueda binaria de rango (usando mergeSort)...");
        ...
        System.out.printf("getGamesByPriceRange (Binaria): %.3f ms, Encontrados: %d juegos\n",
        ...
        System.out.println("Buscando por categoría: " + categoryToSearch);
        ...
        System.out.printf("getGamesByCategory (Lineal): %.3f ms, Encontrados: %d juegos\n",
...
// Binary Search for getGamesByCategory
        ...
        System.out.println("Ordenando por 'category' para búsqueda binaria (usando mergeSort)...");
        ...
        System.out.printf("getGamesByCategory (Binaria): %.3f ms, Encontrados: %d juegos\n",
                durationCategoryBinaryMs, resultsCategoryBinary.size());

        System.out.println("\n--- getGamesByQuality ---");
        int qualityToSearch = 75; // Example quality (your generator: 0-100)
        if (!games_1M_master.isEmpty()) {
            qualityToSearch = games_1M_master.get(datasetSize / 2).getQuality();
        }
        System.out.println("Buscando por calidad: " + qualityToSearch);
...
// Linear Search for getGamesByQuality
        ...
        System.out.printf("getGamesByQuality (Lineal): %.3f ms, Encontrados: %d juegos\n",
...
// Binary Search for getGamesByQuality
        ...
        System.out.println("Ordenando por 'quality' para búsqueda binaria (usando mergeSort)...");
        ...
        System.out.printf("getGamesByQuality (Binaria): %.3f ms, Encontrados: %d juegos\n",
